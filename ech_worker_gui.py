...
class ECHWorkerGUI(Gtk.Window):
    def __init__(self):
        ...
        configs = [
            ...
            {"key": "routing", "label": "分流模式", "default": "global", "required": False},
        ...
        for i, config in enumerate(configs):
            ...
            # 创建输入控件：普通文本输入或分流模式下拉
            if config["key"] == "routing":
                combo = Gtk.ComboBoxText()
                combo.set_hexpand(True)
                combo.set_halign(Gtk.Align.FILL)
                # 使用 id, label 方式存储，让保存时能取到实际参数值
                combo.append("global", "全局")
                combo.append("bypass_cn", "绕中国大陆")
                combo.append("none", "直连")
                combo.append("custom", "自定义")
                # 设定默认值
                try:
                    combo.set_active_id(config.get("default", "global"))
                except Exception:
                    pass
                grid.attach(combo, 1, i, 1, 1)
                self.entries[config["key"]] = combo
            else:
                entry = Gtk.Entry()
        ...
        # 检查可执行文件是否存在
        self.executable_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ech-worker-mod")
        if not os.path.isfile(self.executable_path) or not os.access(self.executable_path, os.X_OK):
            self.status_label.set_text("错误: 找不到可执行文件 ech-worker-mod")
            self.status_label.override_color(Gtk.StateFlags.NORMAL, Gdk.RGBA(1, 0, 0, 1))  # 红色
            self.start_button.set_sensitive(False)
            self.append_log("错误: 找不到可执行文件 ech-worker-mod")
        else:
        ...
        try:
            if os.path.exists(config_file):
                ...
                for key, entry in self.entries.items():
                    if key in config:
                        val = config[key]
                        # 如果是下拉控件（分流模式），使用 set_active_id
                        if isinstance(entry, Gtk.ComboBoxText):
                            try:
                                entry.set_active_id(val)
                            except Exception:
                                # 忽略无效 id
                                pass
                        else:
    ...
    def save_config(self, config_name=None):
        ...
        try:
            config = {}
            for key, entry in self.entries.items():
                # 如果是下拉控件，使用 get_active_id() 获取实际参数值
                if isinstance(entry, Gtk.ComboBoxText):
                    try:
                        val = entry.get_active_id()
                    except Exception:
                        val = None
                    config[key] = val if val is not None else ""
                else:
    ...
    def on_start_clicked(self, widget):
        ...
        for key, entry in self.entries.items():
            ...
            # 支持 Entry 和 ComboBoxText
            if isinstance(entry, Gtk.ComboBoxText):
                value = entry.get_active_id() or ""
            else:
